...
def calculate_topological_metrics(
    ...
    triangles = torch.diagonal(
        torch.matmul(adj_matrix, torch.matmul(adj_matrix, adj_matrix))
    )
    ...
    avg_neighbor_degree = torch.where(
        neighbor_counts > 0,
        neighbor_degrees / neighbor_counts,
        torch.zeros_like(neighbor_degrees),
    )
    ...
    path_lengths = torch.ones((num_nodes, num_nodes)) * float(
        "inf"
...
class UpscaledGraphDataLoader:
    ...
    def __iter__(self):
        for i in range(0, len(self.input_graphs), self.batch_size):
            ...
            input_graphs = Batch.from_data_list(
                [self.input_graphs[idx] for idx in batch_indices]
            )
            output_graphs = Batch.from_data_list(
                [self.output_graphs[idx] for idx in batch_indices]
            )
...
class GraphDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.train_dataset = self._get_data(
            lr_data=self.lr_train, hr_data=self.hr_train
        )
    ...
    def self_train_dataloader(self) -> UpscaledGraphDataLoader:
        return UpscaledGraphDataLoader(
            self.train_dataset[0], self.train_dataset[0], batch_size=self.batch_size
        )

    def train_dataloader(self) -> UpscaledGraphDataLoader:
        return UpscaledGraphDataLoader(
            self.train_dataset[0], self.train_dataset[1], batch_size=self.batch_size
        )

    def self_val_dataloader(self) -> UpscaledGraphDataLoader:
        return UpscaledGraphDataLoader(
            self.val_dataset[0], self.val_dataset[0], batch_size=self.batch_size
        )

    def val_dataloader(self) -> UpscaledGraphDataLoader:
        return UpscaledGraphDataLoader(
            self.val_dataset[0], self.val_dataset[1], batch_size=self.batch_size
        )
